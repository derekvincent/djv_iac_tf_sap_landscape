locals {
  ...
  engress_security_rules = concat(var.security_group_egress_rules)
  ...
  j2ee_instance_security_group = lookup(data.terraform_remote_state.base.outputs.sap_j2ee_security_groups_id_list, var.sap_j2ee_sysnr)
  j2ee_ascs_security_group     = lookup(data.terraform_remote_state.base.outputs.sap_j2ee_security_groups_id_list, join("-", [var.sap_j2ee_ascs_sysnr, "scs"]))
  additional_security_groups = concat(list(local.instance_security_group, local.ascs_security_group,
    local.j2ee_instance_security_group, local.j2ee_ascs_security_group),
  var.additional_security_group_arns)
}
...
module "sap_ansible_input" {
  ...
  ip_address         = module.sap_solman.instance.private_ip
  swap_device        = module.sap_solman.swap_device
